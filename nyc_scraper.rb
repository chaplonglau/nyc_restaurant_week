...
p.workbook do |wb|
  ...
  wb.add_worksheet(:name => 'Restaurant Week Jan 22 2018') do  |ws|
    ws.add_row ['Restaurant Week Jan 22 2018'], :style => title
    ws.add_row
    ws.add_row ['Name', 'Rating', 'Review Count', 'Category', 'url',], :style => header
    
    #YELP QUERY
    masterlist.each_with_index do |restaurant,index|
      response= Yelp.client.search( 'Manhattan, NY', { term: masterlist[index], limit: 1  })
      business=response.businesses[0]
      #ADDING IT INTO OUR WORKSHEET
      if (defined? business.name)
        ws.add_row [business.name, business.rating, business.review_count, business.categories[0][0], business.url]
        puts business.name
      end
...
p.serialize 'nycrestaurantweek.xlsx'
